 ...
 * @version 1.7.4 (Refactored to use Database Class)
 */
...
try {
    // Get the database connection from our new Database class
    ...
    $update_active_stmt = $db->prepare("UPDATE users SET last_active = NOW() WHERE id = ? AND (last_active IS NULL OR last_active < NOW() - INTERVAL 1 MINUTE)");
    ...
    $online_count = 1;
    $online_query = $db->query("SELECT COUNT(id) as online_count FROM users WHERE last_active >= NOW() - INTERVAL 5 MINUTE");
    if ($online_query) {
    ...
    function getUserData($db, $user_id) {
        $stmt = $db->prepare("SELECT username, phone_number, balance, notifications, is_banned, last_bonus_date, avatar, vip_level FROM users WHERE id = ?");
        if (!$stmt) {
            error_log("Get User Data Prepare failed: " . $db->error);
            return null;
        }
    ...
    function processDailyBonus($db, &$user_row, $user_id) {
        $today = date('Y-m-d');
        ...
        $bonus_stmt = $db->prepare("SELECT setting_value FROM settings WHERE setting_key = ?");
        ...
        if ($bonus_amt > 0) {
            ...
            $update_stmt = $db->prepare("UPDATE users SET balance = balance + ?, last_bonus_date = ? WHERE id = ? AND (last_bonus_date IS NULL OR last_bonus_date != ?)");
            ...
            if ($affected_rows > 0) {
                ...
                $noti_stmt = $db->prepare("INSERT INTO system_notifications (user_id, message) VALUES (?, ?)");
                $noti_stmt->bind_param("is", $user_id, $noti_msg);
                $noti_stmt->execute();
                $noti_stmt->close();
                
                $update_noti_stmt = $db->prepare("UPDATE users SET notifications = notifications + 1 WHERE id = ?");
            ...
            $update_date_stmt = $db->prepare("UPDATE users SET last_bonus_date = ? WHERE id = ?");
    ...
    if ($row) {
        ...
        $bonus_alert = processDailyBonus($db, $row, $user_id);
    ...
    $banner_stmt = $db->query("SELECT setting_key, setting_value FROM settings WHERE setting_key IN ('home_banner_url', 'home_banner_url_2', 'home_banner_url_3')");
    $valid_banners = [];
    if ($banner_stmt) {
    ...
    $imp_noti_stmt = $db->prepare("SELECT id, message FROM system_notifications WHERE (user_id IS NULL OR user_id = ?) AND is_important = 1 AND is_read = 0 ORDER BY created_at DESC LIMIT 1");
    ...
    error_log("HomeController critical error: " . $e->getMessage());
